...
public class ChooseAddressFromRuleActivity extends BaseActivity {
...
//		customers_goods = getIntent().getStringExtra("customers_goods");
	...
	private void initData() {
		for (int i = 0; i < mSelectGoodlist.size(); i++) {
			...
			if (i == 0) {
				customerIds = selectGoodBean.getCid();
				productIds = selectGoodBean.getPid();
			} else {
				customerIds = customerIds + "," + selectGoodBean.getCid();
				productIds = productIds + "," + selectGoodBean.getPid();
			}
		}
	...
	public void payBtn(View view) {
		...
		if (checkId == R.id.radio0) {
			NetProxyManager.getInstance().toAddOrderByRule(mHandler, tokenid, customerIds, id, productIds, null,
					card_info, card_content, "1", null);
		} else {
			int selectIndex = adapter.getSelectIndex();
			ExpressAddressBean expressAddressBean = list.get(selectIndex);
			NetProxyManager.getInstance().toAddOrderByRule(mHandler, tokenid, customerIds, id, productIds, null,
					card_info, card_content, "2", expressAddressBean.getId());
		}
	}
	...
	private void parseAddOrder(String result) {
		...
		try {
			...
			i.putExtra("notice_id", id);
			String amount = "";
			if (job.has("amount")) {
				amount = job.getString("amount");
				LogUtil.d("lyp", "amount: " + amount);
			}
			
			if (job.has("unpaid")) {
				amount = job.getString("unpaid");
				LogUtil.d("lyp", "unpaid: " + amount);
			}
			LogUtil.d("lyp", "final amount: " + amount);
