...
import com.lyp.membersystem.adapter.GoodsAdapter;
...
public class GoodsActivity extends BaseActivity implements OnRefreshListener2<ListView> {
	/** 资源对象 */
	...
	private void initView() {
		...
		mAdapter = new GoodsAdapter(goodsList, this, mainHandler);
	...
	private void parseGetProductList(String result) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				goodBean.setIsService(childObj.getString("isService"));
				...
				if (childObj.has("tagIds")) {
					goodBean.setTagIds(childObj.getString("tagIds"));
				}
