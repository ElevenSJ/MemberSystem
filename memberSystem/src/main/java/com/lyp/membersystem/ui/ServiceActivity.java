...
import com.lyp.membersystem.adapter.ServicesAdapter;
...
public class ServiceActivity extends BaseActivity implements OnRefreshListener2<ListView> {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			switch (msg.what) {
			case MessageContants.MSG_GET_SERVICE_GROUP_LIST: {
	...
	private void initView() {
		...
		lv_order.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> adapterView, View view, int position, long arg3) {
				ServiceBean serviceBean = serviceList.get(position - 1);
				Intent intent = new Intent();
				intent.setClass(ServiceActivity.this, ServiceDetailActivity.class);
				intent.putExtra("id", serviceBean.getServiceId());
				intent.putExtra("name", serviceBean.getName());
				intent.putExtra("icon", serviceBean.getIcon());
	...
	private void getData() {
		...
		NetProxyManager.getInstance().toGetServiceGroupList(mHandler, tokenid, mPage + (serviceList.size() / mRow),
				mRow, null);
	}
	...
	private void parseGetServiceList(String result) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				serviceBean.setName(jb.getString("pname"));
				serviceBean.setSname(jb.getString("sname"));
				serviceBean.setIcon(jb.getString("picUrls"));
				serviceBean.setRecordStatus(jb.getString("recordStatus"));
